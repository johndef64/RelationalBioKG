...
def top_competitors(model, emb, test, tail_pool, pos_tail, device, k, batch_rows=200):
            ...
            for b, (h_i, r_i, t_i) in enumerate(chunk):
                for known in pos_tail.get((h_i, r_i), ()):
                    if known != t_i and known in cand_to_idx:
                        scores[b, cand_to_idx[known]] = -float("inf")
...
def main():
    ...
    print(f"[i] {model_name} run{selected_run} | config {config_name} | split_seed {data_seed} | "
    ...
    out = args.out or os.path.join(args.model_folder, f"test_ranks_{args.task}.csv")
    df.to_csv(out, index=False)

    mrr = df["rr_mean"].mean()
    print(f"\n[i] MRR from the dumped ranks: {mrr:.4f}   <- must match the run's test MRR")
    for k in (1, 3, 10):
